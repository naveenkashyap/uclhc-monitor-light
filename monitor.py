...
class FileManager(object):
    ...
    def load_file(filename):
        ...
        if "cache" in filename:
            filename = BASE_PATH_CACHE + threading.current_thread().name + ".json"
        elif "outbox" in filename:
            filename = BASE_PATH_OUTBOX + threading.current_thread().name + ".json"
        f = open(filename, 'r')
...
def run(metricmngr, config, address):
    ...
    log_file = BASE_PATH_LOG + file_name + ".log"
